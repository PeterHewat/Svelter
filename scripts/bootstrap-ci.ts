...
export async function bootstrapCiSecrets(
  ...
  if (!proceed) {
    requireManualAction(
      "Sync dev CI secrets to GitHub",
      [
        "This template requires GitHub Actions secrets for PR CI, merge-to-main staging, and Playwright E2E",
        "Re-run `bun run setup` and confirm the sync prompt",
      ],
      options,
    );
    return;
